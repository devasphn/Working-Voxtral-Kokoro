...
class VoxtralModel:
    ...
    async def process_audio_stream(self, audio_data: torch.Tensor, prompt: str = "Transcribe this audio.") -> str:
        """
        Process streaming audio and return transcription/response - FIXED data type handling
        ...
        try:
            ...
            with self.model_lock:
                # FIXED: Ensure audio_data is writeable and correct dtype
                if not audio_data.data.is_contiguous():
                    audio_data = audio_data.contiguous()
                
                # Convert tensor to numpy with explicit copy to ensure writeability
                ...
                with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as tmp_file:
                    try:
                        ...
                        # Process inputs with FIXED dtype handling
                        inputs = self.processor.apply_chat_template(
                            [openai_message], 
                            return_tensors="pt"
                        )
